...
try:
    ...
    count = 0
    modules = ["httpx", "requests", "pyperclip", "websockets", "dearpygui"]
    for _ in modules:
        count += 1
...
def tab2(): # api
    with dpg.group():
        with dpg.collapsing_header(label="Spam Lightmap Contacts"): # 6
            ...
            dpg.add_input_text(hint="Enter threads", default_value="500", label="Threads", tag="threads", width=150, scientific=True)
